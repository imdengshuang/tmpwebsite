...
class DbManager(object):
    ...
    def getone(self):
        data = self.session.query(ListPage).filter(ListPage.status==0).first()
        data.status = 1
...
def catchhtml(url):
    try:
        ...
        # request.urlretrieve(url, 'cache/2.txt')
...
def insertdb(url,title):
    ...
    if(url.startswith('http://www.jjgirls.com/')):
        db.insert(Article,url,title,0)
    else:
        db.insert(Article,url,title,9)
...
if __name__=='__main__':
    ...
    if len(args)==2:
        ...
        limit = 1
    # res = catchhtml('http://www.jjgirls.com/archive/1')
    # print(res)
    lock = Lock()
    cpu_count = cpu_count()
    p = Pool(cpu_count)
